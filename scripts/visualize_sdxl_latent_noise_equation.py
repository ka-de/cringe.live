"""
Visualize the noise addition equation in SDXL latent space:
q(z_t|z_{t-1}) = N(z_t; √(1-β_t)z_{t-1}, β_tI)

This script creates a visualization showing four panels using the SDXL VAE
...
- Uses SDXL VAE for higher quality and stability
"""
...
def visualize_latent_noise_equation(image_path, num_steps=10, fps=30):
    """
    Create a visualization of the noise equation process in SDXL latent space.
    """
    ...
    # Create animation
    create_animation(frames, output_dir / 'sdxl_latent_noise_equation.mp4', fps=fps)

def main():
    parser = argparse.ArgumentParser(description='Visualize the noise equation process in SDXL latent space')
